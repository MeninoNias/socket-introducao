...
while True:
    ...
    for nSocks in read_sockts:
        if nSocks == server_sock:
            ...
            if isnumber(message['data'].decode('utf-8')) is False:
                message['data'] = 'INFORME SOMENTE NUMEROS'.encode()
                message['header'] = f"{len(message['data']):<{HEAD_LENGTH}}".encode()
            else:
                message['data'] = str(int(message['data'])-1).encode()
                message['header'] = f"{len(message['data']):<{HEAD_LENGTH}}".encode()
